(function () {
  ...
  function renderJpikeBomba(play, context = {}) {
    ...
    const textInputId = `jpike-bomba-text-${playId}`;
    ...
    function validateFields(endDate) {
      return {
        ok: !!endDate,
        error: "Para aprobar o salvar una bomba, fecha fin es obligatoria."
      };
    }
    ...
    setTimeout(() => {
      ...
      async function loadRecurrenceIfNeeded() {
        ...
        try {
          ...
          row.querySelectorAll('[data-role="recurrence-month"]').forEach((input) => {
            input.checked = recurrenceMonthsValue.includes(String(input.value));
          });
      ...
      function renderMode() {
        ...
        if (deadlineRead) {
          deadlineRead.style.display = isRead ? "flex" : "none";
        }

        if (deadlineEdit) {
          deadlineEdit.style.display = isEdit ? "flex" : "none";
        }
      ...
      btnSave?.addEventListener("click", () => {
        ...
        const check = validateFields(payload.endDate);
...
`;
  ...
  window.renderJpikeBomba = renderJpikeBomba;
})();
